...
export function useDiagnosticoViewModel() {
  ...
  const handleVerDiagnosticosCreados = async () => {
    try {
      ...
      const response = await AuthServiceDiagnostico.GetDiagnosticos(CT_ID_INCIDENCIA);
  ...
  const handleCambioEstado = async () => {
    ...
    try {
      await AuthServiceIncidencia.CambioEstado(CT_ID_INCIDENCIA, "7", CT_CEDULA.CT_CEDULA);
      setDiagnostico("");
      setTiempo("");
      setObservaciones("");
      setRequiereCompra("0");
