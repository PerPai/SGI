...
export class AuthServiceIncidencia {
  ...
  static async asignar(incidencia: IncidenciaModelA, asignador: string) {
    try {
      const response = await axios.post(
        `http://localhost:3000/api/incidencia/asignar`,
        incidencia
      );
  ...
  static async CambioEstado(CT_ID_INCIDENCIA: string, CN_ID_ESTADO: string, CT_CEDULA: string) {
    ...
    try {
      const response = await axios.post(
        `http://localhost:3000/api/incidencia/cambio?CT_ID_INCIDENCIA=${CT_ID_INCIDENCIA}&CN_ID_ESTADO=${CN_ID_ESTADO}&CT_CEDULA=${CT_CEDULA}`
      );
      return response.data;
    } catch (error: unknown) {
      console.log(error);
  ...
  static async getAsignado(CT_CEDULA: string) {
    try {
      const response = await axios.get(`http://localhost:3000/api/asignada`, {
        params: {
          CT_CEDULA,
        },
      });
  ...
  static async getDataSupervision(CT_ID_INCIDENCIA: string) {
    ...
    try {
      const response = await axios.get(`http://localhost:3000/api/supervision/data?CT_ID_INCIDENCIA=${CT_ID_INCIDENCIA}`);
      console.log(response.data);
      return response.data;
    } catch (error: unknown) {
      if (axios.isAxiosError(error)) {
  ...
  static async getTerminado() {
    try {
      const response = await axios.get(
        `http://localhost:3000/api/incidencias/terminadas`,
        {}
      );
      return response.data;
    } catch (error: unknown) {
